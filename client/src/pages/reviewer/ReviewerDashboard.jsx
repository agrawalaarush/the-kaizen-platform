...
export default function ReviewerDashboard() {
  ...
  const loadDashboard = async () => {
    try {
      ...
      const response = await getReviewerDashboard(token);
  ...
  const scrollToPending = () => {
    document
      .getElementById("pending-reviews")
      ?.scrollIntoView({
        behavior: "smooth",
      });
  };

  const scrollToReviewed = (filter = "All") => {
    setReviewFilter(filter);

    document
      .getElementById("recently-reviewed")
      ?.scrollIntoView({
        behavior: "smooth",
      });
  };
...
<StatCard
  ...
  onClick={() =>
    navigate("/my-reviewed-ideas?status=Approved")
  }
/>
...
<StatCard
  ...
  onClick={() =>
    navigate("/my-reviewed-ideas?status=Rejected")
  }
/>
...
/>
      ...
      >
        ...
        <h2 className="text-xl font-semibold text-blue-900">
          ...
          {dashboard.pendingIdeas.length > 0
            ? `${dashboard.pendingIdeas.length} Ideas Awaiting Your Review`
      ...
      >
        ...
        <div className="flex items-center justify-between mb-5">
          ...
          <span className="text-sm text-gray-500">
            {dashboard.pendingIdeas.length} Pending
          </span>
        ...
        {dashboard.pendingIdeas.length === 0 ? (
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            {dashboard.pendingIdeas.map((idea) => (
